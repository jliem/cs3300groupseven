 ...
 */
public final class Connection extends UnicastRemoteObject implements
		ConnectionRemote, Identifiable<ConnectionIdentifier> {
	...
	/**
	 * A collection of objects who need to be notified when this connection gets
	 * disconnected.
	 */
	 ...
	 * @param client
	 *            a remote reference to the client object
	 * @throws RemoteException
	 *             if an rmi error occurs
	 */
	public Connection(final ColabServer server, final ColabClientRemote client)
			throws RemoteException {
	...
	/**
	 * Returns the community that is active on this connection. The connection
	 * must be logged in to a community.
	 * 
	...
	/** {@inheritDoc} */
	public void logIn(final CommunityName communityName, final char[] password)
			throws RemoteException {

		// Must be in the Logged In (not yet in a community) state
		if (this.state != ConnectionState.LOGGED_IN) {
			throw new IllegalStateException(
					"Cannot login to community on connection in '" + this.state
							+ "' state");
		}
		...
		Collection<ServerChannel> channels = server.getChannels(communityName);
	...
	public void logOutCommunity() throws RemoteException {
		...
		if (!this.state.hasCommunityLogin()) {
			throw new IllegalStateException(
					"Attempt to log out of community in '" + this.state
							+ "' state");
		}
	...
	public Collection<CommunityName> getAllCommunityNames()
		...
		Collection<CommunityName> communityNames = new ArrayList<CommunityName>(
				communities.size());
	...
	public void joinChannel(final ChannelRemote clientChannel,
		...
		if (serverChannel == null) {
			throw new IllegalStateException("Could not join channel named "
	 ...
	 * @throws RemoteException
	 *             any exception which should be tossed back to the client is
	 ...
	 * 
	 * @param communityName
	 *            the name of the community to retrieve
	 * @return the community
	 * @throws CommunityDoesNotExistException
	 *             if no community with that name exists
	 */
	...
	/**
	 * Ensures that the listener will be notified when this connection gets
	 * disconnected.
	 * 
	 * @param listener
	 *            the object which needs to be notified
	 */
	...
	/**
	 * This method should be called when a fatal RMI exception is thrown and the
	 * connection needs to be aborted.
	 * 
	 * @param e
	 *            the exception which caused the disconnect, used to explain why
	 *            the disconnection is occurring
	 */
	...
	public boolean checkCommunityPassword(CommunityName commName,
			char[] password) throws RemoteException {
		// TODO Auto-generated method stub

		try {
			return server.getCommunity(commName).checkPassword(password);
	...
	public void addUserToCommunity(UserName user, CommunityName comm)
		...
		// TODO Auto-generated method stub
		try {
			server.getCommunity(comm).addMember(user);
		} catch (CommunityDoesNotExistException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
